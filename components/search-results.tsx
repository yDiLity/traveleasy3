...
export function SearchResults({
  ...
  // Мемоизированная функция для сортировки отелей
  const sortedHotels = useMemo(() => {
    console.log('Пересортировка отелей...');
  ...
  // Мемоизированная функция для фильтрации отелей
  const filteredHotels = useMemo(() => {
    console.log('Перефильтрация отелей...');
    return sortedHotels.filter(hotel => {
      ...
      // Фильтрация по удобствам - оптимизированная проверка
      for (const amenity of selectedAmenities) {
        if (amenity === 'wifi' && !hotel.amenities.wifi) return false;
        if (amenity === 'parking' && !hotel.amenities.parking) return false;
        if (amenity === 'pool' && !hotel.amenities.pool) return false;
        if (amenity === 'restaurant' && !hotel.amenities.restaurant) return false;
        if (amenity === 'spa' && !hotel.amenities.spa) return false;
      }

      return true
    });
  }, [sortedHotels, priceRange, minRating, selectedStars, selectedAmenities]);

  // Мемоизированная функция для переключения звезд в фильтре
  const toggleStar = useCallback((star: number) => {
    setSelectedStars(prevStars => {
      if (prevStars.includes(star)) {
        return prevStars.filter(s => s !== star);
      } else {
        return [...prevStars, star];
      }
    });
  }, []);

  // Мемоизированная функция для переключения удобств в фильтре
  ...
  // Мемоизированная функция для сброса всех фильтров
            ...
            <VirtualizedList
              ...
              renderItem={(hotel, index) => (
                <div className="py-2">
                  ...
                  >
                <HotelListItem hotel={hotel} index={index} t={t} />
                  </motion.div>
                </div>
              )}
            />
...
// Мемоизированный компонент для отображения отеля в списке
...
const HotelListItem = memo(({ hotel, index, t }: HotelListItemProps) => {
  return (
  ...
  // Оптимизация рендеринга: перерисовываем только если изменился отель
