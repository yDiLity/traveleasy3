...
// Определяем тип отеля для использования на странице
interface Hotel {
  id: number;
  name: string;
  location: string;
...
export default function HomePage() {
  ...
  // Загружаем отели при монтировании компонента
  useEffect(() => {
    async function fetchHotels() {
      try {
        // Загружаем премиум отели (5 звезд)
        const luxuryResponse = await fetch('/api/hotels/all?stars=5&limit=3')
        const luxuryData = await luxuryResponse.json()

        // Загружаем отели со специальными предложениями
        const specialOffersResponse = await fetch('/api/hotels/all?hasSpecialOffers=true&limit=3')
        const specialOffersData = await specialOffersResponse.json()

        setLuxuryHotels(luxuryData.hotels || [])
        setSpecialOfferHotels(specialOffersData.hotels || [])
      } catch (error) {
    ...
    <div className="min-h-screen bg-transparent">
      ...
      <div className="py-8 text-center hide-on-search">
        <div className="opacity-100">
          <Link href={`/${locale}/hotels`} className="inline-flex items-center px-6 py-3 bg-primary text-white rounded-lg hover:bg-primary/90 transition-colors shadow-md">
            {locale === 'ru' ? 'Посмотреть все отели' : 'View all hotels'}
      ...
      <div className="py-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto hide-on-search">
        <div>
          <div className="my-8">
            ...
            {isLoading ? (
              // Индикатор загрузки для премиум отелей
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {[1, 2, 3].map((i) => (
                  <div key={i} className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 shadow-lg rounded-lg p-4 animate-pulse">
              ...
              // Сообщение, если отели не найдены
              <div className="text-center py-8 bg-gray-50 dark:bg-gray-800 rounded-lg">
                <p className="text-gray-600 dark:text-gray-300">
                  Премиум отели не найдены. Пожалуйста, попробуйте позже.
                </p>
              </div>
            )}
          </div>
          ...
          <div className="my-8">
            ...
            {isLoading ? (
              // Индикатор загрузки для специальных предложений
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {[1, 2, 3].map((i) => (
                  <div key={i} className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 shadow-lg rounded-lg p-4 animate-pulse">
                    <div className="h-36 bg-gray-200 dark:bg-gray-700 rounded-lg mb-4"></div>
              ...
              // Отображение отелей со специальными предложениями
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="text-center py-8 bg-gray-50 dark:bg-gray-800 rounded-lg">
                <p className="text-gray-600 dark:text-gray-300">
                  Отели со специальными предложениями не найдены. Пожалуйста, попробуйте позже.
                </p>
              </div>
            )}
          </div>
